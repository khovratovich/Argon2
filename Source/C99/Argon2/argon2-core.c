...
void InitInstance(Argon2_instance_t* instance_ptr, block* mem_ptr, Argon2_type t, uint32_t p , uint32_t m , uint8_t l ){
    instance_ptr->memory = mem_ptr;   
    instance_ptr->passes = p;  
    instance_ptr->memory_blocks = m;   
    instance_ptr->segment_length = m /(l*ARGON2_SYNC_POINTS);   
    instance_ptr->lane_length = m / l; 
    instance_ptr->lanes = l; 
    instance_ptr->type = t;  
    instance_ptr->Sbox = NULL;    
}


void InitPosition(Argon2_position_t* position_ptr, uint32_t p, uint8_t l , uint8_t s , uint32_t i)
{
    position_ptr->pass=p;
    position_ptr->lane=l;
    position_ptr->slice=s;
    position_ptr->index=i;
}
